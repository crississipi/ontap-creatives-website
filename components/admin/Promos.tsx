...
const Promos = () => {
  ...
  return (
    <div className="w-full h-full grid grid-cols-5 gap-3 relative">
      {addPromo ? (
        <div className="absolute w-full md:relative md:col-span-3 md:h-full rounded-md shadow-md shadow-neutral-200 p-5 bg-white overflow-hidden">
            ...
            <div className="h-full w-full flex flex-col-reverse md:flex-row items-center pt-3 pb-10 gap-5 overflow-hidden">
              <div className="w-full md:w-1/2 h-full grid grid-cols-2 gap-3 py-5 items-center">
            ...
            <div
                className="rounded-sm h-max md:h-full w-full md:w-1/2 bg-white ml-auto shadow-[inset_0_2px_8px_rgba(0,0,0,0.2)] flex flex-col justify-start items-center md:overflow-hidden"
                onDragOver={handleDragOver}
                onDrop={handleDrop}
            >
                {!fileSelected ? (
                <>
                    <div className="w-1/2 md:w-3/4 aspect-square border-2 border-dashed border-neutral-400 flex flex-col items-center justify-center my-20 md:mt-16 relative group transition-all">
        ...
        <button 
        type="button" 
        className="flex items-center gap-2 px-5 py-3 text-sm md:text-base absolute bg-white shadow-md shadow-neutral-200 rounded-md z-30 hover:bg-light-blue focus:bg-violet focus:text-white ease-out duration-200"
      ...
      <div className={`${!addPromo ? 'col-span-full' : 'col-span-2'} h-full flex flex-col gap-3 overflow-x-hidden`}>
        ...
        <div className={`w-full ${addPromo ? 'h-full grid grid-cols-2' : 'h-auto grid grid-cols-2 md:grid-cols-5'} gap-3 overflow-x-hidden`}>
            {Array.from({length: 10}).map((_, i) => (
                <button key={i} type="button" className="bg-white col-span-1 h-min rounded-md flex flex-col p-2 md:p-3 gap-3 shadow-md shadow-neutral-200">
                    <span className="rounded-xs w-full aspect-video bg-white ml-auto shadow-[inset_0_2px_8px_rgba(0,0,0,0.2)] "></span>
                    <div className="flex flex-col">
                        <h4 className="w-full text-left font-semibold text-sm md:text-base text-nowrap">Promo Name</h4>
