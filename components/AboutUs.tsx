...
const AboutUs = ({editable}: EditProps) => {
  ...
  return (
    <section className='h-[100vh] md:h-auto w-full flex flex-col items-center gap-5 text-dark-blue py-16'>
        <div className='md:min-h-[100vh] min-h-1/3 w-full flex items-center justify-center relative'>
        {editable ? 
          <EditableText tag="h1" className='z-20 md:text-9xl text-3xl tracking-wider text-white font-bold text-center' type='input'>
            ABOUT US!
          </EditableText> : 
          <h1 className='z-20 md:text-8xl lg:text-9xl text-5xl tracking-wider text-white font-bold'>ABOUT US!</h1>
        }
            <Image
                height={4096}
                width={4096}
        ...
        <div className='w-6/7 flex flex-col lg:flex-row items-center justify-around gap-5 mt-0 lg:mt-20'>
          <span className='w-full lg:w-2/5 flex flex-col gap-3'>
            ...
            <h3 className='lg:text-xl border p-3 border-light-blue'>To harness the modern technological landscape to build more efficient communities and elevate every user's journey. This dedicated focus on seamless interaction and connection is what ultimately drives sustainable business excellence for our partners.</h3>
          </span>
          <span className='w-full lg:w-2/5 flex flex-col gap-3'>
            <h2 className='col-span-full text-center text-3xl font-extrabold lg:font-normal lg:text-6xl uppercase flex justify-center'>
              <span>val</span>
            ...
            <h3 className='lg:text-xl border p-3 border-light-blue'>To deliver dynamic innovations through user-friendly technology that integrates seamlessly into daily life. With a single tap, we guarantee precision and reliability, transforming user satisfaction into an enhanced and trusted brand reputation.</h3>
          </span>
        ...
        <div className='w-6/7 lg:w-3/4 h-max grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 lg:mt-10 gap-5'>
          ...
          {coreValues.map((val,i) => (
            <div key={`core-values-${i}`} className='flex p-3 px-5 items-center gap-3 border border-light-blue'>
              ...
              <span className='h-full w-full flex flex-col'>
                <strong className='uppercase text-xl font-extrabold'>{val.label}</strong>
                <h3>{val.text}</h3>
              </span>
            </div>
          ))}
