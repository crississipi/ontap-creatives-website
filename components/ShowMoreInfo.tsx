...
const ShowMoreInfo = ({
  ...
  // Use product object for all data
  const finalProductName = product?.name || "";
  const finalProductDesc = product?.description ? (
    <p>{product.description}</p>
  ) : (
    <p></p>
  );
  const finalFrontImg = product?.frontUrl || "";
  const finalBackImg = product?.backUrl || "";
  const finalVariableFrontImg = product?.variableFrontImg || "";
  const finalVariableBackImg = product?.variableBackImg || "";
  const finalImgUrl = product?.imgUrl || "";
  ...
  const cartOptions = useClickOutside<HTMLDivElement>(
    ...
    const fetchFeedbacks = async () => {
      try {
        ...
        const response = await fetch(
          `https://ontap-creatives-website.vercel.app/api/feedbacks/${product.productID}`
        );
  ...
  const handleBuyAction = async () => {
    ...
    if (userData) {
      ...
      const productData = {
        ...
        subtotal:
          priceOption === "ontap"
            ? product.price.ontap * quantity
            : product.price.custom! * quantity,
        variable: variable,
  ...
  const handleAddToCart = async () => {
    ...
    if (userData) {
      try {
        ...
        const productData = {
          productID: product.productID, // The product ID from your product data
          clientID: user?.clientID, // The logged-in user's client ID
          quantity: quantity, // The quantity selected by user
          subtotal:
            priceOption === "ontap"
              ? product.price.ontap * quantity
              : product.price.custom * quantity, // Calculate subtotal
          logo: priceOption === "ontap" ? "OnTap" : "Custom",
        };
        ...
        if (response.ok) {
          ...
          setShow(true);
          setIcon("error");
          setMessage(`Error: ${result.error || "Failed to add item to cart"}`);
        }
      } catch (error) {
        // Network or other errors with toast
        setShow(true);
        setIcon("error");
  ...
  const CustomImg: Record<string, string[]> = {
    ...
    "Elite Digital Business Card": [
      "/images/card-4/custom-front.png",
      "/images/card-4/custom-back.png",
    ],
  };
  ...
  return (
    <div className="h-full w-full fixed top-16 md:top-0 left-0 bg-white/15 backdrop-blur-sm z-100 flex items-center justify-center">
      {show && <Toast icon={icon} message={message} />}
      <div
        ref={!showLogin ? clickRef : null}
        className="h-full w-full md:h-4/5 lg:h-3/4 md:w-4/5 lg:w-[90%] bg-white shadow-md rounded-2xl grid grid-cols-1 lg:grid-cols-10 lg:gap-1 lg:p-5 relative overflow-x-hidden lg:overflow-auto"
      >
        <div className="flex items-center gap-2 group transition-all duration-200 absolute top-5 left-5 z-50 bg-white/30 backdrop-blur-md pr-3 rounded-r-full">
          <span className="rounded-full text-blue text-3xl flex items-center justify-center hover:ring-2 hover:ring-blue ease-out duration-200">
            <RiQuestionFill />
          </span>
            ...
            >
              <span
                className={`h-full aspect-square ${
                  animationState
                    ? "ml-auto bg-violet"
                    : "mr-auto bg-neutral-400"
                } rounded-full  group-focus:bg-violet ease-out duration-200`}
              ></span>
            </button>
          </div>
        </div>
        <button
          type="button"
          className="md:hidden absolute text-2xl z-50 right-5 top-5 hover:text-rose-300 focus:text-rose-500 focus:scale-125 ease-out duration-200"
          onClick={() => setInquireItem(false)} // This should work with the new function
        >
          <HiOutlineX />
        </button>
        <div className="col-span-1 md:col-span-3 h-full w-full flex lg:flex-col gap-2 perspective-[1000px] lg:overflow-hidden">
                      ...
                      >
                        {priceOption === "ontap" ? (
                          ...
                          <div className="h-full w-full relative flex items-center justify-center">
                            ...
                            <div
                              className={`${logoSize} h-1/2 absolute inset-0 top-1/2 ${
                                finalProductName ===
                                "Elite Digital Business Card"
                        ...
                        finalProductName !== "Elite Digital Business Card" ? (
                          ...
                          finalProductName ===
                            ...
                            <div className="h-full w-full relative flex items-center justify-center">
                              ...
                              />
                              <div
                                className={`${logoSize} h-1/2 absolute inset-0 top-1/2 left-2/5 -translate-y-1/2  z-20`}
                              >
                                <Image
                                  src={
                                    fileInfo?.preview
                                                              ...
                                                              "Elite Digital Business Card"
                                                                ? logoSize ===
                                                                  ...
                                                                  : logoSize ===
                                                                    ...
                                                                    "scale-120"
                                                                : logoSize
                                                            } h-2/5 absolute inset-0 top-1/2 ${
                                  finalProductName ===
                                  "Elite Digital Business Card"
                                    ? "-left-2/5 -translate-y-1/2"
                                    : "left-1/2 -translate-1/2"
                                                              ...
                                                              "Elite Digital Business Card"
                                                                ? logoSize ===
                                                                  ...
                                                                  : logoSize ===
                                                                    ...
                                                                    "scale-120"
                                                                : logoSize
                                                            } h-2/5 absolute inset-0 top-1/2 left-2/5 -translate-y-1/2 z-20 opacity-60`}
                                >
                                  <Image
                                    src={
                                      fileInfo?.preview
          ...
          <div className="hidden lg:block h-1/3 w-full rounded-lg border border-neutral-300">
            ...
            <div className="w-full flex flex-col p-3 gap-2">
              ...
              <div className="w-full grid grid-cols-10 items-center gap-3 text-sm font-extrabold">
                <p className="col-span-1">2</p>
                <div className="col-span-8 w-full h-3 flex items-center rounded-full bg-neutral-300 overflow-hidden"></div>
        ...
        <div className="col-span-7 h-full grid grid-cols-5 p-5 pb-0 pr-0 pl-0 lg:pl-5 lg:pt-3 gap-y-5 lg:overflow-x-hidden items-start relative">
          <div className="col-span-full lg:col-span-3 w-full h-full flex flex-col justify-start px-5 lg:px-0 relative">
            <h1 className="col-span-full text-2xl lg:text-4xl font-extrabold text-black px-5 lg:px-0">
              {finalProductName}
            </h1>
            <span className="col-span-full leading-5 text-lg px-5 lg:px-0 lg:my-10">
              {finalProductDesc}
            </span>
            ...
            <span className="font-extrabold flex items-center mb-5">
              {product.price.ontap === 0 ? (
                ...
                <>
                  ...
                  <p className="hidden lg:block text-4xl">
                    {(priceOption === "ontap"
                      ? product.price.ontap * quantity
                      : product.price.custom! * quantity
                  ...
                  <p className="lg:hidden text-4xl">
                    {(priceOption === "ontap"
                      ? product.price.ontap
                      : product.price.custom!
                    ).toLocaleString("en-US", {
            ...
            <div className="md:hidden lg:flex w-full flex-col h-max lg:min-h-1/2 lg:shadow-transparent fixed lg:relative bottom-0 mb-16 md:mb-0 left-0 z-50 transition-all ease-out duration-700">
              <div
                className={`hidden lg:flex w-full bg-neutral-100 md:bg-white flex-col mb-16`}
              >
                {finalVariableBackImg && finalVariableFrontImg && (
                  <div className="w-full flex flex-col p-5 pb-0 md:p-0">
                    ...
                    <div className="flex gap-1 mt-1 mb-3">
                      <button
                        type="button"
                        className={`max-w-28 flex flex-col gap-1 p-2 px-3 text-base md:text-sm border border-neutral-200 group ${
                          variable === "white"
                            ? "bg-violet text-white"
                            : "hover:bg-neutral-100"
                        } hover:bg-neutral-100 focus:bg-violet focus:text-white transition-all ease-out duration-200`}
                        onClick={(e) => {
                          setVariable("white");
                          e.preventDefault();
                          e.stopPropagation();
                ...
                {finalImgUrl === "" && (
                  <div
                    className={`w-full flex flex-col px-5 md:p-0 ${
                      finalProductName !== "Polyvinyl Business Card" && "pt-5"
                    }`}
                  >
                    ...
                    <div className="flex gap-1 mt-1 mb-3">
                      <button
                        type="button"
                        className={`max-w-28 flex flex-col gap-1 p-2 text-base md:text-sm px-3 border border-neutral-200 group ${
                          priceOption === "ontap"
                            ? "bg-violet text-white"
                            : "hover:bg-neutral-100"
                        } focus:bg-violet focus:text-white transition-all ease-out duration-200`}
                        onClick={(e) => {
                          setPriceOption("ontap");
                          e.preventDefault();
                          e.stopPropagation();
                        }}
                      >
                      ...
                      <button
                        type="button"
                        className={`max-w-28 flex flex-col text-base md:text-sm gap-1 p-2 px-3 border border-neutral-200 group  ${
                          priceOption === "custom"
                            ? "bg-violet text-white"
                            : "hover:bg-neutral-100"
                        } focus:bg-violet focus:text-white transition-all ease-out duration-200`}
                ...
                {priceOption === "custom" && (
                  <div className="w-full flex flex-col md:flex-row gap-2 md:items-center mb-3 px-5 pt-3 p-0">
                    ...
                    >
                      ...
                      <span className="hidden absolute left-full ml-2 group-hover:block bg-white text-black w-max">
                        Please attach your custom logo here.
                      </span>
                    </button>
                    <div className="flex gap-1 items-center text-sm">
                      ...
                      <span className="flex items-center gap-0.5">
                        <button
                          type="button"
                          className={`flex flex-col gap-1 p-1 text-sm px-3 border border-neutral-200 group ${
                            logoSize === "scale-100"
                              ? "bg-violet text-white"
                              : "hover:bg-neutral-100"
                          } focus:bg-violet focus:text-white transition-all ease-out duration-200`}
                          onClick={(e) => {
                            setLogoSize("scale-100");
                            e.preventDefault();
                            e.stopPropagation();
                          }}
                        >
              ...
              <AnimatePresence mode="wait">
                {showCustomize && (
                  <motion.div
                    ...
                    ref={cartOptions}
                    className={`2xl:hidden w-full bg-neutral-100 md:bg-white flex flex-col shadow-[0px_-3px_14px_#00000099]`}
                  >
                    {finalVariableBackImg && finalVariableFrontImg && (
                      <div className="w-full flex flex-col p-5 pb-0 md:p-0">
                        <h2 className="font-semibold">Variations</h2>
                    ...
                    {finalImgUrl === "" && (
                      <div
                        className={`w-full flex flex-col px-5 md:p-0 ${
                          finalProductName !== "Polyvinyl Business Card" &&
                          "pt-5"
                        }`}
                      >
                        <h2 className="font-semibold">Logo Style</h2>
              ...
              <div
                className={`hidden lg:flex justify-end md:justify-start gap-3 w-full pt-3 md:py-0 absolute bottom-0 bg-white p-3 md:p-0 z-30 ${
                  !showCustomize &&
                  "shadow-[0px_-3px_14px_#00000099] lg:shadow-md lg:shadow-transparent"
                }`}
              >
                <button
                  type="button"
                  className="px-5 py-3 rounded-md border border-neutral-300 text-black text-xl hover:bg-neutral-300 focus:text-white focus:bg-blue ease-out duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                <button
                  type="button"
                  className="pl-10 pr-7 md:pl-5 md:pr-3 py-3 rounded-md flex items-center gap-3 bg-dark-blue font-bold text-white hover:bg-violet focus:bg-footer-bg ease-out duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  onClick={handleBuyAction}
                  disabled={checkingAuth}
                >
                  Buy now <RiArrowRightLine className="text-xl" />
                </button>
              </div>
            </div>
          </div>
              ...
              {loadingFeedbacks ? (
                ...
                feedbackData.feedbacks.map((feedback) => (
                  <div
                    ...
                    className="flex flex-col w-full px-2 border-b border-neutral-200"
                  >
                    <div className="w-full flex items-center gap-3">
                      ...
                      <span className="font-semibold">
                        <h3>
                          {feedback.client.clientName || "Anonymous User"}
                        </h3>
        ...
        <AccountSignIn
          ...
          onSuccess={() => {
            ...
            checkAuthStatus().then((userData) => {
              ...
              if (userData) {
                console.log(
                  "User logged in successfully, you can now proceed with purchase"
                );
